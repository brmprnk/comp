...
def get_args():
    ...
    general_group.add_argument(
        ...
        help="A text file containing all input BAM files with one BAM file per line, a single BAM file, or a directory containing BAM files.\n"
    )
    general_group.add_argument(
        '-o', '--output_dir', type=str, default='./extracted_features',
    ...
    general_group.add_argument(
        ...
        help="Minimum mapping quality for reads to be considered. Default: [30]"
    )
    general_group.add_argument(
        '--min_frag_len', type=int, default=51,
        help="Minimum fragment length to consider. Default: [51]"
    )
    general_group.add_argument(
        '--max_frag_len', type=int, default=400,
        help="Maximum fragment length to consider. Default: [400]"
    )
    general_group.add_argument(
        '-g', '--genome_version', type=str, default='hg38', choices=['hg19', 'hg38'],
        help="Genome version of input BAM files (hg19/hg38). Default: [hg38]"
    )
    general_group.add_argument(
        '--bl', '--blacklist', type=str, default='hg38/hg38_GCcorrection_ExclusionList.merged.sorted.bed',
        help="Path to a blacklist BED file to filter out regions. Default: [hg38/hg38_GCcorrection_ExclusionList.merged.sorted.bed]\n"
    )
    general_group.add_argument(
        '-b', '--bed_file', type=str,
        help="A BED3 file specifying the regions to extract features, or a directory containing BED files or a .txt file with BED file paths.\n"
    )
    general_group.add_argument(
    ...
    general_group.add_argument(
        '--gc', action='store_true', default=True,
        help="Use the GCFix output file to incorporate GC Bias correction. Default: [True]"
    )
    general_group.add_argument(
        '--gc_file', type=str,
        help="Path to the GCFix output file for GC Bias correction."
    )
    ...
    cna_group.add_argument(
        '-B', '--bin_size', type=int, default=1000, choices=[10, 50, 500, 1000],
        help="Bin size in kilobases (10, 50, 500, or 1000). Default: [1000]"
    )
    cna_group.add_argument(
        '--CNA_params', type=str,
        help="Additional parameter string for CNA analysis."
    )

    # --- NOF specific options ---
    nof_group = parser.add_argument_group('Options specific for Nucleosome Occupancy and Fuzziness (NOF)')
    nof_group.add_argument(
        '--NOF_params', type=str,
        help="Additional parameter string for NOF analysis."
    )
    ...
    wps_group.add_argument(
        '-x', '--min_len_long', type=int, default=120,
        help="Min fragment length for long fragments WPS. Default: [120]"
    )
    wps_group.add_argument(
        '-X', '--max_len_long', type=int, default=180,
        help="Max fragment length for long fragments WPS. Default: [180]"
    )
    wps_group.add_argument(
        '-w', '--win_size_long', type=int, default=120,
        help="Window size for long fragments WPS. Default: [120]"
    )
    wps_group.add_argument(
        '-m', '--min_len_short', type=int, default=35,
        help="Min fragment length for short fragments WPS. Default: [35]"
    )
    wps_group.add_argument(
        '-M', '--max_len_short', type=int, default=80,
        help="Max fragment length for short fragments WPS. Default: [80]"
    )
    wps_group.add_argument(
        '-W', '--win_size_short', type=int, default=16,
        help="Window size for short fragments WPS. Default: [16]"
    )

    # --- EM specific options ---
    em_group = parser.add_argument_group('Options specific for End Motif (EM)')
    em_group.add_argument(
        '-f', '--fasta', type=str, default='hg38/ref.fa',
        help="Reference genome in FASTA format."
    )
    em_group.add_argument(
        '-k', '--kmer_size', type=int, default=3,
        help="K-mer size for motif extraction. Default: [3]"
    )

    # --- NP specific options ---
    np_group = parser.add_argument_group('Options specific for Nucleosome Profile (NP)')
    np_group.add_argument(
        '-l', '--sites_path', type=str,
        help="Directory containing a list of files with each file for a set of sites."
    )
    ...
    pfe_group.add_argument(
        '-T', '--tss_info', type=str,
        help="A TAB-delimited TSS information file without any header."
    )
    pfe_group.add_argument(
        '--PFE_params', type=str,
        help="Additional parameter string for PFE analysis."
    )

    # --- TSSC specific options ---
    tssc_group = parser.add_argument_group('Options for TSS Coverage (TSSC)')
    tssc_group.add_argument(
        '-u', '--upstream', type=int, default=1000,
        help="Number of base pairs upstream of TSS. Default: [1000]"
    )
    tssc_group.add_argument(
        '-d', '--downstream', type=int, default=1000,
        help="Number of base pairs downstream of TSS. Default: [1000]"
    )
    tssc_group.add_argument(
        '-S', '--tss_file', type=str,
        help="A BED6 file specifying the coordinates of TSSs."
    )
    tssc_group.add_argument(
        '-n', '--normalization', type=str, default='RPKM',
        choices=['RPKM', 'CPM', 'BPM', 'RPGC'],
        help="Normalization method for bamCoverage. Default: [RPKM]"
    )
    tssc_group.add_argument(
        '--bamCoverage_params', type=str,
        help="Additional parameter string for bamCoverage."
    )
    tssc_group.add_argument(
        '--multiBigwigSummary_params', type=str,
        help="Additional parameter string for multiBigwigSummary."
    )
...
def run_analysis(args):
    ...
    print(f"Requested features: {', '.join(sorted(list(features_to_extract)))}")
